...
def render_thermal_tab():
    ...
    with col2:
        ...
        section = st.number_input("Sección del Conductor (mm²)", value=1.0, step=1.0, key="th_sec_unique")

        t_amb = st.slider("Temperatura Ambiente (°C)", 0, 60, 40, key="th_tamb_unique")

        v_sys = st.number_input("Tensión del Sistema (V)", value=1, key="th_volt_unique")
    ...
    with col_c1:
        ...
        p_load = st.number_input("Potencia Activa (kW)", value=1.0)

    with col_c2:

        cos_phi = st.number_input("Factor de Potencia (cos φ)", value=0.1, min_value=0.01, max_value=1.0)
...
def render_sizing_tab():
    ...
    with c_in1:
        ...
        P_load = st.number_input("Carga (kW)", value=1.0)

        L_line = st.number_input("Longitud (m)", value=1.0)
